...
export function BudgetLayoutPreview({ budget, settings }: { budget: PreviewBudget; settings: Settings; layoutId?: string | null }) {
  ...
  const proposalLogoSrc = settings.proposal_logo_url ?? settings.logo_url;
      ...
      <div className="grid gap-0 lg:grid-cols-[0.72fr_1fr]">
        <div className="flex min-h-80 items-center justify-center bg-[#3f5d70] p-8 text-white">
          <div className="flex h-64 w-full max-w-md items-center justify-center">
            {logoSrc ? <img src={logoSrc} alt="Logo da empresa" className="max-h-56 max-w-full object-contain" /> : <div className="text-center text-2xl font-bold">{settings.company_name}</div>}
          </div>
        </div>
        ...
        <div className="relative min-h-80 bg-white">
          ...
          <div className="relative p-6 pb-24">
            <header className="relative mb-3 h-28">
              <div className="absolute left-2 top-12 w-[215px] whitespace-nowrap text-[13px] leading-[1.18]">
                <p><strong>Cliente:</strong> {budget.clients?.name ?? "-"}</p>
                <p><strong>Empresa:</strong> {settings.company_name}</p>
                <p><strong>Prazo de Execução:</strong> {budget.execution_deadline || "-"}</p>
                <p><strong>Proposta:</strong> {budget.budget_number}</p>
              </div>
              <div className="absolute inset-x-0 top-4 mx-auto flex h-24 w-48 items-center justify-center bg-white p-1">
                {proposalLogoSrc ? <img src={proposalLogoSrc} alt="Logo da proposta comercial" className="h-full w-full object-contain" /> : <span className="text-xs font-semibold text-[#3f5d70]">Logo</span>}
              </div>
              <div className="absolute right-0 top-9 text-base">Página: 1/2</div>
            </header>
            ...
            <div className="space-y-4">
              {firstGroups.map((group) => (
                <div key={group.id} className="overflow-hidden border border-slate-950 text-[10px]">
                  <div className="bg-[#163f4a] py-1 text-center font-bold uppercase text-white">Descritivo Orçamentário</div>
                  ...
                  {group.budget_items.slice(0, 3).map((item, index) => (
                    <div key={item.id} className="grid grid-cols-[0.7fr_1.1fr_3fr_0.8fr_1fr_1.1fr] border-t border-slate-900">
                      ...
                      <Cell>{item.name}</Cell>
                      <Cell>{formatUnit(item.unit)}</Cell>
