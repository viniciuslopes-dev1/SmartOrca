-- Clean Auth/RLS setup for Supabase SQL Editor.
-- Use this file if 0005_auth_workspaces_rls.sql fails with "no language specified".
-- Do not delete older SQL files; just run this one after the base schema exists.
...
begin
  ...
  values (
    ...
    'Valores sujeitos a validacao tecnica em campo.'
  )
...
create policy "budget_status_history_select_member" on public.budget_status_history
for select to authenticated
using (
  exists (
    select 1 from public.budgets b
...
create policy "budget_status_history_insert_member" on public.budget_status_history
for insert to authenticated
with check (
  exists (
    select 1 from public.budgets b
...
create policy "budget_exports_select_member" on public.budget_exports
for select to authenticated
using (private.is_workspace_member(workspace_id));

create policy "budget_exports_insert_member" on public.budget_exports
for insert to authenticated
with check (private.is_workspace_member(workspace_id));
